...
@section('content')
    <div class="record-edit">
        <a href="{{ route('records.index') }}" class="app-back-btn">
            <svg width="18" height="18" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                stroke="currentColor">
        ...
        <div class="record-edit-card">
            ...
            <div class="record-insert-alert" role="status">
                <strong>Before you save</strong> — Check that the fields are correct. Click <em>Add record</em>, then
                confirm in the popup to insert this row.
            </div>
            <form action="{{ route('records.store') }}" method="POST" id="record-create-form">
                            ...
                            @foreach ($columns as $col)
                                <tr class="edit-table-tr">
                                    ...
                                    <td class="edit-table-td">
                                        ...
                                        @endphp
                                        <input type="{{ $inputType }}" name="{{ $requestKey }}" id="field-{{ Str::slug($col) }}"
                                            value="{{ $val }}" placeholder="{{ $placeholder }}" @if ($inputMaxlength)
                                            maxlength="{{ $inputMaxlength }}" @endif @if ($inputPattern)
                                            pattern="{{ $inputPattern }}" @endif @if ($inputTitle) title="{{ $inputTitle }}"
                                            @endif @if ($inputMin !== null) min="{{ $inputMin }}" @endif @if ($inputStep !== null)
                                            step="{{ $inputStep }}" @endif @if ($inputMode) inputmode="{{ $inputMode }}" @endif
                                            @if (!in_array($col, $manualCreateOptionalColumns, true)) required
                                            aria-required="true" @endif class="edit-table-input">
                                        @error($col)
                ...
                <div class="record-edit-footer">
                    <button type="submit" class="record-edit-btn record-edit-btn-primary" id="record-create-submit">Add
                        record</button>
    ...
    <div id="record-insert-confirm" class="record-insert-confirm record-insert-confirm-hidden" aria-hidden="true">
        ...
        <div class="record-insert-confirm-panel" role="dialog" aria-modal="true"
            aria-labelledby="record-insert-confirm-title">
            ...
            <p class="record-insert-confirm-text">This will save a new row to the database. You can add images after saving.
            </p>
            <div class="record-insert-confirm-actions">
                <button type="button" class="record-insert-confirm-btn record-insert-confirm-btn-secondary"
                    id="record-insert-confirm-cancel">Cancel</button>
                <button type="button" class="record-insert-confirm-btn record-insert-confirm-btn-primary"
                    id="record-insert-confirm-yes">Yes, insert</button>
            </div>
        </div>
    </div>
    <script>
